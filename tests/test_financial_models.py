...
def test_explicit_fdm_reasonable_price(is_call):
    price = financial_models.explicit_fdm(grid_size, time_steps, Smax, 1, K, r, sigma_mid, is_call, S0)
    assert isinstance(price, float)
...
def test_implicit_fdm_reasonable_price(is_call):
    price = financial_models.implicit_fdm(grid_size, time_steps, Smax, 1, K, r, sigma_mid, is_call, S0)
    assert isinstance(price, float)
    assert price >= -1e-12
...
def test_crank_nicolson_fdm_reasonable_price(is_call):
    price = financial_models.crank_nicolson_fdm(grid_size, time_steps, Smax, 1, K, r, sigma_mid, is_call, False, S0)
    assert isinstance(price, float)
    assert price >= -1e-12
...
def test_american_psor_fdm_reasonable_price(is_call):
    price = financial_models.american_psor_fdm(grid_size, time_steps, Smax, 1, K, r, sigma_mid, is_call, 1.2, 10000, 1e-6, S0)
    assert isinstance(price, float)
    assert price >= -1e-12
...
def test_exponential_integral_fdm_reasonable_price(is_call):
    price = financial_models.exponential_integral_fdm(grid_size, Smax, 1, K, r, sigma_mid, is_call, S0)
    assert isinstance(price, float)
    assert price >= -1e-12
...
def test_fractional_fdm_reasonable_price(is_call):
    price = financial_models.fractional_fdm(grid_size, time_steps, Smax, 1, K, r, sigma_mid, is_call, beta, S0)
    assert isinstance(price, float)
    assert price >= -1e-12

def test_fractional_beta_close_to_zero():
    with pytest.raises(ValueError):
        financial_models.solve_fdm("fractional", grid_size, time_steps, Smax, 1, K, r, sigma_mid, True, 1e-9, False, S0)

def test_fractional_beta_close_to_one():
    with pytest.raises(ValueError):
        financial_models.solve_fdm("fractional", grid_size, time_steps, Smax, 1, K, r, sigma_mid, True, 1 - 1e-9, False, S0)
...
def test_available_functions():
    expected = {
        ...
        "fractional_fdm", "normal_pdf", "normal_cdf", "solve_fdm"
    }
...
def test_fdm_extreme_spot_prices(S0_test, method):
    price = financial_models.solve_fdm(method, grid_size, time_steps, Smax, 1, K, r, sigma_mid, True, 0.5, False, S0_test)
    assert price >= -1e-12

# Volatility sensitivity test with small epsilon tolerance
def test_fdm_volatility_sensitivity():
    price_low_vol = financial_models.solve_fdm("explicit", grid_size, time_steps, Smax, 1, K, r, sigma_low, True, 0.5, False, S0)
    price_high_vol = financial_models.solve_fdm("explicit", grid_size, time_steps, Smax, 1, K, r, sigma_high, True, 0.5, False, S0)
...
def test_fdm_interest_rate_sensitivity():
    price_low_r = financial_models.solve_fdm("explicit", grid_size, time_steps, Smax, 1, K, 0.0, sigma_mid, True, 0.5, False, S0)
    price_high_r = financial_models.solve_fdm("explicit", grid_size, time_steps, Smax, 1, K, 0.2, sigma_mid, True, 0.5, False, S0)
...
def test_american_option_price_ge_european():
    price_american = financial_models.solve_fdm("american", grid_size, time_steps, Smax, 1, K, r, sigma_mid, True, 0.5, False, S0)
    price_european = financial_models.solve_fdm("crank", grid_size, time_steps, Smax, 1, K, r, sigma_mid, True, 0.5, False, S0)
